...
function createSSLCertificate() {
    ...
    if (fs.existsSync(keyFile) && fs.existsSync(certFile)) {
        console.log('✅ SSL sertifikası bulundu');
        return {
        ...
        return new Promise((resolve, reject) => {
            openssl.on('close', (code) => {
                if (code === 0) {
                    ...
                    console.log('❌ SSL sertifikası oluşturulamadı, HTTP modu kullanılacak');
                    resolve(null);
                }
            });

            openssl.on('error', (error) => {
                console.log('❌ OpenSSL bulunamadı, HTTP modu kullanılacak');
                resolve(null);
            });
        });
    } catch (error) {
        console.log('❌ OpenSSL hatası, HTTP modu kullanılacak');
        return null;
    }
}
...
function createManualCertificate() {
    ...
    console.log('📝 Manuel sertifika oluşturuluyor...');
...
-----END CERTIFICATE-----`;
        ...
        console.log('✅ Manuel sertifika oluşturuldu (geliştirme amaçlı)');
        ...
        console.log('❌ Manuel sertifika oluşturulamadı:', error.message);
        return null;
    }
}
...
async function startServer() {
    ...
    if (sslOptions) {
        try {
            ...
            console.log('❌ HTTPS başlatılamadı:', error.message);
    ...
    function encrypt(text, key, additionalData = '') {
        try {
            ...
            console.log('❌ Şifreleme hatası:', error.message);
            return text;
        }
    }
    ...
    function decrypt(encryptedText, key, additionalData = '') {
        try {
            ...
            console.log('❌ Şifre çözme hatası:', error.message);
            return encryptedText;
        }
    }
    ...
    function createMessage(id, username, message, time, sessionKey) {
        ...
        return {
            id,
            username,
            message: encrypt(message, sessionKey, additionalData),
            time,
            readBy: new Set(),
            createdAt: Date.now()
        };
    ...
    io.on('connection', (socket) => {
        ...
        console.log(`🔌 ${wsProtocol.toUpperCase()} bağlantısı: ${socket.id} (${clientIp})`);
        
        // Rate limiting kontrolü
        if (!checkRateLimit(socket.id)) {
            console.log(`🚫 Rate limit aşıldı: ${socket.id}`);
            socket.disconnect();
            return;
        }
        ...
        socket.on('verify key', (inputKey) => {
            ...
            if (verifyKey(inputKey, userSalt)) {
                ...
                console.log(`🔐 Güvenli kimlik: ${socket.id.substring(0, 8)}... (${wsProtocol.toUpperCase()})`);
            } else {
        ...
        socket.on('set username', (username) => {
            ...
            console.log(`👤 Kullanıcı: ${username} (${wsProtocol.toUpperCase()})`);
        });

        socket.on('chat message', (data) => {
            if (!authenticatedUsers.has(socket.id) || !users[socket.id]) {
                socket.emit('error', 'Yetkisiz erişim');
                return;
            }
            ...
            console.log(`🔒 Güvenli mesaj: ${user.username} (${wsProtocol.toUpperCase()})`);
        });
        ...
        socket.on('disconnect', () => {
            ...
            if (users[socket.id]) {
                ...
                console.log(`👋 Oturum sonlandı: ${username} (${wsProtocol.toUpperCase()})`);
            }
        });
    });
    ...
    server.listen(PORT, '0.0.0.0', () => {
        ...
        if (isHttps) {
            ...
            console.log('   ⚠️  Self-signed sertifika kullanılıyor');
            console.log('   🔐 Tarayıcıda güvenlik uyarısını kabul edin');
            console.log('   ✅ Yerel ağda güvenle kullanılabilir');
        } else {
            console.log('⚠️  GÜVENLIK UYARISI:');
            console.log('   ❌ HTTP modu - transport layer şifreleme yok');
            console.log('   💡 OpenSSL kurarak HTTPS etkinleştirin');
        }
        console.log('============================================');
    });
